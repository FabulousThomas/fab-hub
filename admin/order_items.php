...
$sql = "SELECT * FROM order_items ORDER BY order_id DESC LIMIT $start_from, $num_per_page";
$get_items = $conn->query($sql);
...
if (isset($_POST['btn-delete-order-items'])) {
   $item_id = $_POST['item_id'];

   $stmt1 = $conn->prepare("DELETE FROM order_items WHERE item_id=? LIMIT 1");
   $stmt1->bind_param("i", $item_id);

   if (!$stmt1->execute()) {
      echo "Something went wrong";
   } else {
      flashMsg("message", "Deleted");
      redirect("order_items.php?page=" . $page . "#orders");
   }
}
...
<body>
                              ...
                              <?php if ($get_items) : ?>
                                 <?php foreach ($get_items as $item) : ?>
                                    <tr class="align-items-center">
                                       ...
                                       <td><?= $item['user_id'] ?></td>
                                       <td><img src="../assets/img/<?= $item['product_image'] ?>"></td>
                                       ...
                                       <td>
                                          <div class="dropdown open bg-light">
                                             ...
                                             <div class="dropdown-menu shadow">
                                                <a class="dropdown-item btn-sm btn-edit-order-items" data-toggle="modals" data-target="#edit-order-items-modals">Edit</a>

                                                <form method="POST">
                                                   <input type="hidden" name="item_id" value="<?= $item['item_id']; ?>">
                                                   <input type="submit" name="btn-delete-order-items" value="Delete" class="dropdown-item btn-sm">
                                                </form>
                                             </div>
                                          </div>
                                       </td>
   ...
   <!-- <script>
      $(document).ready(function () {
         $('.btn-edit-order-items').on('click', function () {
            $('#edit-order-items-modal').modal('show');
            ...
            // console.log(data);
            $('#order_items_id').val(data[0]);
            $('#order_items_cost').val(data[1]);
            $('#order_items_status').val(data[2]);
            $('#user_id').val(data[3]);
            $('#user_phone').val(data[4]);
            $('#user_email').val(data[5]);
            $('#order_items_date').val(data[8]);
         });
      });
   </script> -->
</body>
